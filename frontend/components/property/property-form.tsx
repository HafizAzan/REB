...
import { PropertyMediaField } from '@/components/property/property-media-field';
...
export function PropertyForm({ property }: { property?: Property }) {
  ...
  const [media, setMedia] = useState<PropertyMedia[]>(
    property?.images.map((image) => ({
      ...
      kind: image.kind ?? (isVideoUrl(image.url) ? 'VIDEO' : 'IMAGE'),
    })) ?? [],
  );
  ...
  return (
    <form onSubmit={onSubmit} className="space-y-4">
      ...
      <PropertyMediaField items={media} onChange={setMedia} />
